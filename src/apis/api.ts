...
export const getFrameData = async ({ frameId }: { frameId: string | undefined }): Promise<any> => {
    const result = await axios.get(`${process.env.REACT_APP_API}/editor/frame/A1/${frameId}`, getConfigure());
    return result.data;
};

// frame data 수정
export const patchFrameData = async ({ frameId, body }: { frameId: string | undefined; body: any }): Promise<any> => {
    const result = await axios.patch(`${process.env.REACT_APP_API}/editor/frame/A1/${frameId}`, body, getConfigure());
    return result.data;
};
...
export const getImage = async (): Promise<GETIMAGE[]> => {
    const result = await axios.get(`${process.env.REACT_APP_API}/editor/image/A1`, getConfigure());
    return result.data;
};
...
export const getText = async ({ textId = '' }: { textId?: string }): Promise<GETTEXT[]> => {
    const result = await axios.get(`${process.env.REACT_APP_API}/editor/text/A1/${textId}`, getConfigure());
    return result.data;
};
...
export const getSound = async (): Promise<GETSOUND[]> => {
    const result = await axios.get(`${process.env.REACT_APP_API}/editor/sound/A1`, getConfigure());
    return result.data;
};
...
export const getMasterFrame = ({ subjectCode = 'A1' }: { subjectCode: string }): Promise<GETMASTERFRAME[]> =>
    axios.get(`${process.env.REACT_APP_API}/editor/frame/master/${subjectCode}`, getConfigure()).then((r) => r.data);
