...
export default function ObjectSelect({ open, onClose, master }: { open: boolean; onClose: () => void; master?: boolean }) {
        ...
        if (data.imageDivisionCode === _divisionCode.image.background) {
            ...
            if (selectImage && selectImage.image) {
                ...
                if (isSelected)
                    setSelectImage({
                        ...selectImage,
                        image: selectImage.image.filter((el) => el.src !== data.imageId)
                    });
                else
                    setSelectImage({
                        ...selectImage,
                        image: selectImage.image.concat(updateData)
                    });
            } else {
    ...
    const onSaveClick = async () => {
        ...
        if (selectImage && selectImage.background) {
            await drawBackground({
                ...
                data: {
                    id: selectImage.background.id,
                    type: 'background',
                    imageDivisionCode: selectImage.background.imageDivisionCode
                }
            });
        }
        if (selectImage && selectImage.image) {
                ...
                selectImage.image.map((el) =>
                    drawImage({
                        ...
                        data: {
                            id: el.id,
                            type: el.type,
                            imageDivisionCode: el.imageDivisionCode,
                            frame: el.frame
                        },
        ...
        setDivisionCode(master ? '05' : '04');
        onClose();
    };

    useEffect(() => {
        if (master) setDivisionCode('05');
        else setDivisionCode('04');
    }, [master]);
    ...
    return (
        <MetaModal title={'Objects'} open={open} onClose={onClose} onSave={onSaveClick}>
